...
bool SettingsManager::loadSettings() {
  ...
  size_t read_total =
      this->eeprom->readBytes(EEPROM_START_ADDR, (void *)buf, EEPROM_READ_LEN);
...
bool SettingsManager::printSettings(char *buf, size_t len) {
  // Calculate the amount of characters needed for each of the settings, their
  // names, and some formating character (4) Like :, ' ', and \r\n for each
  // character as well as before the first string.
  ...
  snprintf(buf, len,
           "\r\n%s: %s\r\n%s: %s\r\n%s: %s\r\n%s: %s\r\n%s: %s\r\n%s: %s\r\n",
           SSID_ID, this->settings->ssid, PASSWORD_ID, this->settings->password,
           MQTT_SERVER_ID, this->settings->mqtt_server, DEVICE_ID_ID,
           this->settings->device_id, DATA_TOPIC_ID, this->settings->data_topic,
           STATUS_TOPIC_ID, this->settings->status_topic);
  return true;
}

bool SettingsManager::updateSetting(const char *name, const char *value,
                                    int value_len) {
  ...
  // Verify the length of the value is not greater than the max length of the
  // setting
  ...
  if (buf == NULL) {
    this->terminal->debugln("Failed to allocate memory for the buffer when "
