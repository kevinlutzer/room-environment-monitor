...
WiFiController::WiFiController(WiFiClass *wifi, Terminal *terminal,
...
void WiFiController::setupSNTP() {
  const char *time_zone =
      "CET-1CEST,M3.5.0,M10.5.0/3"; // TimeZone rule for Europe/Rome including
...
bool WiFiController::setupWiFi() {
  ...
  if (wifipass == NULL) {
    this->terminal->debugln(
        "Failed to get the password from the settings manager");
    return false;
  }
  ...
  do {
    ...
    if (count % 5 == 0) {
      this->terminal->debugf("WiFi connection status: %s\r\n",
                             wifi_status == WL_CONNECTED ? "connected"
                                                         : "not connected");
    }
    count++;
    delay(100);
